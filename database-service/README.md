...
- **Relationship Queries**: Handles complex queries for risk-control-question mappings
- **Search Functionality**: Full-text search across all entities
- **Capability Analysis**: Capability scenario analysis and gap detection
...
- **`api/capability_scenarios.py`**: Capability scenario analysis endpoints
...
| `CAPABILITY_CONFIG_DB_PATH` | Path to capability config database | Auto-detected |
...
- `GET /api/questions` - List all questions (with pagination and filtering)
  - Query parameters: `limit`, `offset`, `category`
- `GET /api/questions/summary` - Questions summary with counts
- `GET /api/question/{question_id}` - Get specific question details
- `GET /api/managing-roles` - Get distinct managing roles from questions
...
- `GET /api/capabilities` - List all capabilities (with pagination and filtering)
  - Query parameters: `limit`, `offset`, `capability_type`, `domain`
- `GET /api/capabilities/{capability_id}` - Get specific capability details
- `GET /api/capability-tree` - Get capability tree with controls and risks
- `GET /api/capability-unique-controls` - Get unique controls for capabilities
...
The service includes capability scenario management endpoints (via router):

- `GET /api/capability-scenarios` - List capability scenarios
- `POST /api/capability-scenarios` - Create a new scenario
- `GET /api/capability-scenarios/{scenario_id}` - Get scenario details
- `PUT /api/capability-scenarios/{scenario_id}` - Update scenario
- `DELETE /api/capability-scenarios/{scenario_id}` - Delete scenario
- `POST /api/capability-selections` - Bulk update capability selections
- `GET /api/capability-selections/{scenario_id}` - Get selections for scenario
- `POST /api/capability-analysis` - Analyze capability coverage
...
pytest tests/test_capability_analysis.py
...
- **`questions`**: Interview questions
  - `question_id`, `question_text`, `category`, `topic`, `managing_role`
...
- **`capability_control_mapping`**: Capability-control relationships
...
### Capability Configuration Database

The service also manages a separate SQLite database for capability scenarios:

- **`capability_scenarios`**: User-defined capability scenarios
- **`capability_selections`**: Capability selections per scenario
- **`control_selections`**: Control selections per scenario
...
- **SQLite** is used for both main data and capability configuration
