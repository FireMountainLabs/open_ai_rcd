...
# Add parent and shared-services directory to path for imports
...
def test_client(mock_config_manager, sample_database):
    ...
    try:
        ...
        from app import app, reinitialize_repositories

        # Reinitialize repositories with the test database
...
def database_service_process():
    ...
    try:
        ...
        process = subprocess.Popen(
            ["python", str(Path(__file__).parent.parent / "app.py")],
...
def launcher_process():
    ...
    try:
        ...
        process = subprocess.Popen(
            [str(Path(__file__).parent.parent.parent / "launch_dataviewer"), "--dev"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
