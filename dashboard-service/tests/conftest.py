...
try:
    ...
    env_defaults = config_manager.get_config_value("env_defaults", {})
...
def patch_api_client_methods():
    ...
    def _patch_methods(mock_client):
        ...
        methods_to_patch = [
            ...
            'get_questions', 'get_questions_summary', 'get_definitions', 'get_relationships', 
            'search', 'get_stats', 'get_file_metadata', 'get_risk_detail', 'get_control_detail',
            'get_question_detail', 'get_managing_roles', 'health_check'
        ]
...
def mock_database_api_client():
    ...
    mock_client.get_questions.return_value = {
        "questions": [
            {"id": "Q-001", "text": "Test Question 1", "category": "Security"},
            {"id": "Q-002", "text": "Test Question 2", "category": "Privacy"},
        ],
        "total": 2,
    }
    ...
    mock_client.get_gaps.return_value = {
        "summary": {
            ...
            "unmapped_risks": 5,
            "unmapped_controls": 2,
            "risks_without_questions": 3,
            "risks_without_questions_pct": 12.0,
        },
        "gaps": [{"risk_id": "RISK-001", "missing_controls": ["CTRL-001"]}] * 25,
        "risks_without_questions": [
            {"risk_id": "R.AIR.001", "risk_title": "Test Risk", "risk_description": "Test"}
        ],
        "unmapped_risks": [],
        "unmapped_controls": [],
        "unmapped_questions": [],
    ...
    mock_client.get_last_updated.return_value = {
        ...
        "questions": {"last_updated": "2024-01-01T00:00:00Z"},
        "risk_control_links": [],
        "question_risk_links": [],
    ...
    mock_client.get_file_metadata.return_value = {
        "files": {
            ...
            "questions": {"file": "questions.xlsx", "last_modified": "2024-01-01T00:00:00Z"},
        }
    }
    ...
    mock_client.get_question_detail.return_value = {
        "question": {
            "id": "Q-001",
            "text": "Test Question Detail",
            "category": "Security",
        }
    }
...
@pytest.fixture
def mock_capability_auth():
    """Mock authentication for capability scenario routes.
    
    This fixture patches the get_user_id function closure in the routes module
    to return user_id = 1, simulating an authenticated user.
    
    Since get_user_id is a local function, we patch request.current_user
    in the routes module where it's imported.
    """
    ...
    # Patch request in the routes module
    # The routes module imports request from flask, so we patch it there
    with patch("routes.capability_scenarios.request") as mock_request:
        # Make the mock behave like Flask's request object
        type(mock_request).current_user = PropertyMock(return_value=mock_user)
        mock_request.method = "GET"
        mock_request.args = {}
        mock_request.get_json = MagicMock(return_value=None)
        mock_request.is_json = False
        
        # Also patch the require_auth decorator to be a no-op
        # by patching it in app.py where it's passed to the blueprint
        with patch("app.require_auth", lambda f: f):
            yield mock_user
