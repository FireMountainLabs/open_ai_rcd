...
from routes.capability_scenarios import create_capability_scenarios_blueprint
...
class DatabaseAPIClient:
    ...
    def get_questions(self, limit: int = 100, offset: int = 0, category: str = None) -> Dict[str, Any]:
        """Get questions from database service."""
        try:
            params = {"limit": limit, "offset": offset}
            if category:
                params["category"] = category
    ...
    def get_questions_summary(self) -> Dict[str, Any]:
        """Get questions summary with counts for dashboard tables."""
        try:
            response = self.session.get(f"{self.base_url}/api/questions/summary")
            response.raise_for_status()
            return response.json()
        except Exception as e:
            logger.error(f"Failed to fetch questions summary: {e}")
            return {"details": []}
    ...
    def get_question_detail(self, question_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/api/question/{question_id}")
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    def get_managing_roles(self) -> Dict[str, Any]:
        """Get distinct managing roles from questions."""
        try:
            response = self.session.get(f"{self.base_url}/api/managing-roles")
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    def get_capabilities(
        ...
        """Get capabilities from database service."""
        try:
            params = {"limit": limit, "offset": offset}
            if capability_type:
            ...
            response = self.session.get(f"{self.base_url}/api/capabilities", params=params)
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    def get_capability_detail(self, capability_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/api/capabilities/{capability_id}")
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    def get_capability_tree(self) -> Dict[str, Any]:
        """Get capability tree with controls and risks."""
        try:
            response = self.session.get(f"{self.base_url}/api/capability-tree")
            response.raise_for_status()
            return response.json()
        except Exception as e:
            logger.error(f"Failed to fetch capability tree: {e}")
            return []
...
# Register capability scenario routes - no auth required
capability_scenarios_bp = create_capability_scenarios_blueprint(DATABASE_URL, lambda f: f, False)
app.register_blueprint(capability_scenarios_bp)
