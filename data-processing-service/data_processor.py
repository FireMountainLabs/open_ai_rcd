...
class DataProcessor:
    ...
    def __init__(
        ...
        self.capability_extractor = CapabilityExtractor(config_manager)
        ...
        self.question_risk_mapping_df: Optional[pd.DataFrame] = None
        self.question_control_mapping_df: Optional[pd.DataFrame] = None
        self.capability_control_mapping_df: Optional[pd.DataFrame] = None
    ...
    def extract_data(self) -> None:
        ...
        self.questions_df = self._extract_data_type(data_sources, "questions", "question")
        self.definitions_df = self._extract_data_type(data_sources, "definitions", "definition")
        self.capabilities_df = self._extract_data_type(data_sources, "capabilities", "capability")
    ...
    def _extract_data_type(
        ...
        Args:
            ...
            entity_type: Type of entity (risk, control, question, definition)
    ...
    def _extract_mappings(self) -> None:
        ...
        # Extract question mappings if questions are available
        if self.questions_df is not None:
            # Find the question file for mapping extraction
            config = self.config_manager.load_config()
            data_sources = config.get("data_sources", {})

            if "questions" in data_sources:
                question_config = data_sources["questions"]
                question_filename = question_config.get("file")
                if question_filename:
                    question_file = self._resolve_file_path(question_filename, "questions")
                    if question_file:
                        (
                            self.question_risk_mapping_df,
                            self.question_control_mapping_df,
                        ) = self.mapping_extractor.extract_question_mappings(
                            question_file, self.risks_df, self.controls_df
                        )
        ...
        # Create capability-control mappings
        if self.capabilities_df is not None:
            self.capability_control_mapping_df = self.capability_extractor.extract_capability_control_mappings(
                self.capabilities_df
            )
    ...
    def extract_data_adaptive(self, excel_path: Path, entity_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            if entity_type == "risk":
                ...
                return self.question_extractor.extract(excel_path)
            elif entity_type == "definition":
                return self.definitions_extractor.extract_definitions(excel_path)
            elif entity_type == "capability":
                return self.capability_extractor.extract(excel_path)
            else:
                raise ValueError(f"Unknown entity type: {entity_type}")
    ...
    def populate_database(self) -> None:
        ...
        # Insert capabilities data if available
        if "capabilities" in data_sources and self.capabilities_df is not None:
            capability_timestamp = self._get_file_timestamp(self.data_dir / data_sources["capabilities"]["file"])
            self.database_manager.insert_data("capabilities", self.capabilities_df, capability_timestamp)
        ...
        if self.question_risk_mapping_df is not None:
            self.database_manager.insert_data("question_risk_mapping", self.question_risk_mapping_df)
        if self.question_control_mapping_df is not None:
            self.database_manager.insert_data("question_control_mapping", self.question_control_mapping_df)
        if self.capability_control_mapping_df is not None:
            self.database_manager.insert_data("capability_control_mapping", self.capability_control_mapping_df)
    ...
    def _print_summary(self) -> None:
        ...
        if self.capabilities_df is not None:
            print(f"Capabilities extracted: {len(self.capabilities_df)}")
        if self.risk_control_mapping_df is not None:
        ...
        if self.capability_control_mapping_df is not None:
            print(f"Capability-Control mappings: {len(self.capability_control_mapping_df)}")
